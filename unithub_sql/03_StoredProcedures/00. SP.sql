...
BEGIN
	...
	ELSE
		SELECT 'Unknown validation' AS Message, -6 AS Response;
    END IF;
END //
...
BEGIN
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Exception catch: SP_User_Register' AS Message, -1 AS Response;
    END;
    
    IF EXISTS (SELECT 1 FROM User WHERE Email = p_email) THEN
        SELECT 'Account with this email already exists.' AS Message, -2 AS Response;
    ELSEIF EXISTS (SELECT 1 FROM User WHERE Username = p_username) THEN
        SELECT 'Username already exists.' AS Message, -3 AS Response;
    ELSE
...
BEGIN
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Exception catch: SP_User_Get_User_List' AS Message, -1 AS Response;
    END;
    
    IF NOT EXISTS (SELECT 1 FROM User WHERE Role = 'User') THEN
        SELECT 'No data found' AS Message, -2 AS Response;
    ELSE
